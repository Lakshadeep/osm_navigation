            ...
            if last_area_type == area.type and area.type == "corridor":
                ...
                if len(area.waypoints) > 0:
                    ...
                    if exit_wp.type == 'door' and area.type:
                        topological_actions[len(
                            topological_actions) - 1].exit_feature_id = area.waypoints[len(area.waypoints) - 1].id
                        topological_actions[
                            len(topological_actions) - 1].exit_type = exit_wp.type
            else:
                if last_area_type == 'corridor' and area.type == 'junction':
                    topological_actions[
                        len(topological_actions) - 1].exit_feature_id = area.id
                    topological_actions[
                        len(topological_actions) - 1].exit_type = 'junction_entry'
                elif last_area_type == 'junction' and area.type == 'corridor':
                    topological_actions[
                        len(topological_actions) - 1].exit_feature_id = area.id
                    topological_actions[
                        len(topological_actions) - 1].exit_type = 'junction_exit'
                ...
                if len(area.waypoints) > 0 and area.type != 'junction':
                    ...
                    if exit_wp.type == 'door':
                        ta.exit_feature_id = area.waypoints[
                    ...
                    ta.heading = euler_from_quaternion(
                        [orientation.x, orientation.y, orientation.z, orientation.w])[2] * 180 / 3.1457
